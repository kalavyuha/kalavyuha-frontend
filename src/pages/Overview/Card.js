...
// const CardList = ({ data = [], isLoading, buisnessType }) => {
//   const navigate = useNavigate();
//   const [loading, setLoading] = useState(true);
//   const [favourites, setFavourites] = useState([])
//   const [updatingFavourite, setUpdatingFavourite] = useState(null); // Track which item is being updated
...
//     setUpdatingFavourite(businessId); // Show loading state for this specific item

//     try {
//       // Use favourite item ID in delete endpoint
...
const CardList = React.memo(({ data = [], isLoading, buisnessType }) => {
  ...
  // Updated getAmenityIcon function with better icon matching
  ...
  // Simple amenities processing function that removes duplicates
  const processAmenities = (businessFacilities) => {
    ...
    Object.entries(facilities).forEach(([key, value]) => {
      if (typeof value === 'boolean' && value === true) {
        // Skip non-amenity fields
        if (['BusinessId', 'CreatedBy', 'CreatedOn', 'UpdatedBy', 'UpdatedOn'].includes(key)) {
          return;
        }
