            ...
            <Grid container spacing={20}>
                <Grid item xs={12} sm={7}>
                    ...
                    <Box sx={{ columnCount: { xs: 1, sm: 2 }, columnGap: 2 }}>
                        {[...Array(8)].map((_, index) => (
                            <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: 1, breakInside: 'avoid' }}>
...
const AboutUs = ({ 
        ...
        Object.entries(facilities).forEach(([key, value]) => {
            if (typeof value === "object" && value !== null) {
                Object.entries(value).forEach(([subKey, subValue]) => {
                    flatFacilities.push({ key: subKey, label: facilityLabels[subKey] || subKey, available: subValue });
                });
            } else if (typeof value === "boolean") {
                flatFacilities.push({ key, label: facilityLabels[key] || key, available: value });
            }
        });
    }
    ...
    const hasData = buisnessName || description || (facilities && Object.keys(facilities).length > 0) || (processedTiming && processedTiming.length > 0);
    ...
    return (
        <Box sx={{ padding: 4 }}>
           ...
           <Grid container spacing={20}>
                {flatFacilities.length > 0 && <Grid item xs={12} sm={7}>
                    <Typography
                        variant="h6"
                        sx={{
                    ...
                    >
                        {flatFacilities && flatFacilities.map((item, index) => (
                            <ListItem
                                key={index}
                                sx={{
                                    display: 'flex',
                            ...
                            >
                                <ListItemIcon sx={{ minWidth: 32 }}>
                                    {item.available ? <Done color="success" /> : <Clear color="error" />}
                                </ListItemIcon>
                                <ListItemText
                                    primary={item.label}
                ...
                {processedTiming && processedTiming.length > 0 && <Grid item xs={12} sm={flatFacilities.length > 0 ? 5 : 12}>
                    <Typography
                        variant="h6"
                        sx={{
                            fontSize: { xs: "1rem", sm: "1.2rem" },
