...
const LeftPanel = ({
  ...
  }, [firstName, email, formData.businessRole, formData.businessProfile, formData.teamPresence, formData.services, formData.businessHours]);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', bgcolor: '#e2e6ea', minHeight: {xs: "35vh", md:'90vh'}, margin: 2, borderRadius: '16px', padding: 2.5 }}>
      ...
      <Button 
        startIcon={<Send />} 
        onClick={handleNavigateToSupport}
        sx={{ textTransform: 'capitalize', color: 'black', fontWeight: 'bold', fontSize:{xs:13, sm:14} }}
        >
        Shoot us an email
      </Button>
        <Typography variant="body2" sx={{ mb: 2 }}>Raise a query and let our friendly team help you via support page.</Typography>
      ...
      {isMobile && (
       ...
       <Box
        sx={{
          ...
          maxHeight: (isSignIn || businessRoleForm) && (!isMobile || isExpanded) ? '200px' : '0px',
          opacity: (isSignIn || businessRoleForm) && (!isMobile || isExpanded) ? 1 : 0,
        }}
      >
        ...
        >
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: (isSignIn || formData.agreeTerms) && (!isMobile || isExpanded) ? '200px' : '0px',
              opacity: (isSignIn || formData.agreeTerms) && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: ((businessRoleForm && selectedBusinessRole) || (isSignIn && formData.businessRole)) && (!isMobile || isExpanded) ? '100px' : '0px',
              opacity: ((businessRoleForm && selectedBusinessRole) || (isSignIn && formData.businessRole)) && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
            {((businessRoleForm && selectedBusinessRole) || (isSignIn && formData.businessRole)) && (
              <>
                <Typography variant="h6" sx={{ mb: 0, mt: 2, fontWeight: 'bold' }}>Business Category</Typography>
                <Typography variant="body2" sx={{ mb: 2, fontSize: 12, wordBreak: 'break-word' }}>
                  <b>Selected Role:</b> {selectedBusinessRole || formData.businessRole}
                </Typography>
              </>
            )}
          </Box>
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: (isSignIn && formData.businessInfoCompleted) && (!isMobile || isExpanded) ? '300px' : '0px',
              opacity: (isSignIn && formData.businessInfoCompleted) && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: (isSignIn && formData.teamInfoCompleted) && (!isMobile || isExpanded) ? '400px' : '0px',
              opacity: (isSignIn && formData.teamInfoCompleted) && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
                    ...
                    {formData.teamMembers?.length > 0 ? (
                      formData.teamMembers.map((member, index) => (
                        <li key={member.id} style={{ marginBottom: '8px' }}>
                          ...
                          <b>Role:</b> {Array.isArray(member.role) ? member.role.join(', ') : member.role || 'None'}
                        </li>
                      ))
                    ) : (
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: isSignIn && formData && Array.isArray(formData.services) && formData.services.length > 0 && (!isMobile || isExpanded) ? '500px' : '0px',
              opacity: isSignIn && formData && Array.isArray(formData.services) && formData.services.length > 0 && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
          ...
          <Box
            sx={{
              transition: 'max-height 0.3s ease-in-out, opacity 0.3s ease-in-out',
              overflow: 'hidden',
              maxHeight: isSignIn && formData && formData.businessHours && (!isMobile || isExpanded) ? '600px' : '0px',
              opacity: isSignIn && formData && formData.businessHours && (!isMobile || isExpanded) ? 1 : 0,
            }}
          >
                ...
                <Typography variant="body2" sx={{ mb: 2, ml: 1.5, fontSize: 12, wordBreak: 'break-word', overflowWrap: 'break-word' }}>
                  ...
                  <ul style={{ listStyleType: 'none', paddingLeft: 0, marginTop: 8 }}>
                    {[
                      ...
                      const dayStatus = formData.businessHours.daysStatus?.[day.id];
                      const getDisplayTime = () => {
                        if (!dayStatus || !dayStatus.isOpen) return "Closed";
                        
                        if (formData.businessHours.scheduleType === "selected_hours") {
                          return `${dayStatus.startTime} ${dayStatus.startMeridian} - ${dayStatus.endTime} ${dayStatus.endMeridian}`;
                        } else if (formData.businessHours.scheduleType === "appointment") {
                          return "By appointments only";
                        } else if (formData.businessHours.scheduleType === "always_open") {
                      ...
                      return (
                        ...
                        >
                          <b style={{ marginLeft: -10 }}>{day.name}:</b> {getDisplayTime()}
                        </li>
                      );
                    })}
                  </ul>
